...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("share-slack-btn")?.addEventListener("click", async () => {
    try {
      const summaryEl = document.getElementById("dash-summary");
      const text = summaryEl?.textContent?.trim() || "";
      if (!text || text === "Waiting for conversation to begin...") {
      ...
      const slackMessage = `*Meeting Summary*\n\n${text}\n\n_Generated by Late-Meet_`;
      await navigator.clipboard.writeText(slackMessage);
      showToast("Slack message copied to clipboard!", "success");
    } catch {
      showToast("Failed to copy for Slack", "error");
    }
  });
