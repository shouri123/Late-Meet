...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  audioBtn?.addEventListener('click', async () => {
    ...
    function handleDashboardAudioError(err: any) {
      ...
      if (audioBtn) {
        ...
        audioBtn.textContent = (err.message || String(err)).length > 30 ? 'Error — Retry' : (err.message || 'Error');
        setTimeout(() => {
          if (audioBtn) {
  ...
  document.getElementById('export-btn')?.addEventListener('click', async () => {
    try {
      ...
      if (state.decisions?.length) {
        markdown += `## Decisions\n`;
        state.decisions.forEach((d: Decision) => markdown += `- ${d.text}${d.by ? ` — ${d.by}` : ''}\n`);
        markdown += '\n';
      }
  ...
  async function loadSavedSessions() {
    try {
      ...
      container.innerHTML = sessions.map((s: State) => {
        const date = new Date((s as any).savedAt || s.startTime || Date.now()).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
        const time = new Date((s as any).savedAt || s.startTime || Date.now()).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
  ...
  function exportSessionMarkdown(session: State) {
    ...
    if (session.decisions?.length) {
      md += `## Decisions\n`;
      session.decisions.forEach((d: Decision) => md += `- ${d.text}${d.by ? ` — ${d.by}` : ''}\n`);
      md += '\n';
    }
